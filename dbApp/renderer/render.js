...
function currentHome() {
    document.getElementById("server").innerText = config.getServer();
    document.getElementById("db").innerText = config.getDB();
    document.getElementById("table").innerText = config.getTable();
}

function setServer() {
    const newServer = document.getElementById("inputServer").value;
    config.setServer(newServer);
    document.getElementById("server").innerText = config.getServer();
}

function setDB() {
    const newDB = document.getElementById("inputDB").value;
    config.setDB(newDB);
    document.getElementById("db").innerText = config.getDB();
}

function setTable() {
    const newTable = document.getElementById("inputTable").value;
    config.setTable(newTable);
    document.getElementById("table").innerText = config.getTable();
}

function currentUpload() {
    document.getElementById("currFolder").innerText = config.getDataFolder();
    document.getElementById("ldt").innerText = config.getldt();
}

function resetDate() {
    config.setldt('1980-01-01');
    document.getElementById("ldt").innerText = config.getldt();
    console.log("date reset");
}

function setFolder() {
    const newFolder = document.getElementById("inputFolder").value;
    config.setDataFolder(newFolder)
    document.getElementById("currFolder").innerText = config.getDataFolder();
}
            ...
            if (folder.indexOf("Oms_Debug_Log") != -1) {
                ...
                fs.readdirSync(path + '\\' + folder).forEach(file => {
                    ...
                    if (compDates(dt, ldt) && file.indexOf("oms_debug") != -1 && file.indexOf("oms_debug_filtered") == -1) {
                        time = `${file.slice(11, 13)}:${file.slice(14, 15)}0:00`;
                        ...
                        text.forEach(line => {
                            ...
                            if(clordid === '0') {
                                ...
                                query += `INSERT INTO ${table} ([dt], [time], [cl], [se], [server], [si], [clordid], [epochat9_1], [epochat9_2], [omsdebugts1], [omsdebugts2], [thandler1], [thandler2], [timegap]) VALUES ('${dt}', '${time}', '${cl}', '${se}', '${server}', '${si}', '${clordid}', '${str.slice(0, 4)}', '${str.slice(4, str.length)}', '${arr[0]}', '${arr[1]}', '${arr[2]}', '${arr[3]}', '${arr[4]}');\n`;
                                si++;
                                prev = BigInt(curr);
                            }
                            else if(clordid != '' && clordid != '0') {
                                query += `UPDATE ${table} SET trigresp1='${arr[2]}' , trigresp2='${arr[3]}' WHERE (dt='${dt}' AND epochat9_1='${str.slice(0, 4)}' AND epochat9_2='${str.slice(4, str.length)}' AND cl='${cl}' AND se='${se}' AND server='${server}' AND clordid='${clordid}');\n`;
                            }
                        });
                ...
                fs.readdirSync(path + '\\' + folder).forEach(file => {
                    ...
                    if (compDates(dt, ldt) && file.indexOf("request") != -1) {
                        ...
                        text.forEach(line => {
                            ...
                            str = epoch.toString();
                            if(clordid != '') {
                                query += `UPDATE ${table} SET reqsttime1='${arr[0]}' , reqsttime2='${arr[1]}' , security='${arr[2]}' , series='${arr[3]}' , code='${arr[4]}' , buysell='${arr[5]}' , quantity='${arr[6]}' , price='${arr[7]}' WHERE (dt='${dt}' AND epochat9_1='${str.slice(0, 4)}' AND epochat9_2='${str.slice(4, str.length)}' AND cl='${cl}' AND se='${se}' AND server='${server}' AND clordid='${clordid}');\n`;
                            }
                        });
                    }
                        ...
                        text.forEach(line => {
                            ...
                            str = epoch.toString();
                            if(clordid != '') {
                                query += `UPDATE ${table} SET recvtime1='${arr[0]}' , recvtime2='${arr[1]}' , transcode='${arr[2]}' , transdesc='${arr[3]}' , errcode='${arr[4]}' , errdesc='${arr[5]}' , tstatus='${arr[6]}' , exordid1='${arr[7]}' , exordid2='${arr[8]}' , stream='${arr[9]}' , timestmp3_1='${arr[10]}' , timestmp3_2='${arr[11]}' WHERE (dt='${dt}' AND epochat9_1='${str.slice(0, 4)}' AND epochat9_2='${str.slice(4, str.length)}' AND cl='${cl}' AND se='${se}' AND server='${server}' AND clordid='${clordid}');\n`;
                            }
                        });
                        i++;
                    }
...
async function uploadData() {
    try {
        ...
        let cl, se, query = '', temp;
        let ldt = config.getldt();
        let path = config.getDataFolder();
        const con = {
            connectionString: `Driver=SQL Server;Server=${config.getServer()};Database=${config.getDB()};Trusted_Connection=true;`
        };
        fs.readdirSync(path).forEach(folder => {
